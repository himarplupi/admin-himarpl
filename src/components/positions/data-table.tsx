...
import { PositionDeleteAlertDialog } from "./position-delete-alert";
import type {
  VisibilityState,
  ColumnFiltersState,
  RowSelectionState,
...
export function DataTablePositions() {
  const utils = api.useUtils();
  const positions = api.position.all.useQuery();
  ...
  const table = useReactTable({
    ...
    meta: {
      onUpdateRows: async () => {
        await utils.position.invalidate();
      },
      onDeleteRows: async () => {
        await utils.position.invalidate();
        ...
        <div className="my-4 h-10 rounded-md border">
          <Card className="h-full">
            <CardContent className="flex h-full items-center gap-x-2 p-0 px-1 text-muted-foreground">
              ...
              <PositionDeleteAlertDialog
                onDelete={table.options.meta?.onDeleteRows}
                positionIds={getRowIdSelection()}
              >
      ...
      <div className="w-[82vw] rounded-md border sm:w-[80vw] md:w-[85vw] lg:w-full">
        <Table>
          ...
          <TableBody ref={parent}>
            ...
            {table.getRowModel()?.rows.length < 1 && !positions.isLoading && (
              <TableRow>
                <TableCell
